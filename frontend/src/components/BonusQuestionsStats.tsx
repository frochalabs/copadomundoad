...
export function BonusQuestionsStats({ stats }: BonusQuestionsStatsProps) {
  ...
  return (
    <div className="w-full relative my-16 flex flex-col gap-12">
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-12 relative z-10 flex flex-col gap-16">
        {stats.map((activeStat, statIndex) => (
          <motion.section
            key={activeStat.id || statIndex}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.8, delay: statIndex * 0.2 }}
          ...
          >
            ...
            >
              {activeStat.ranking.map((item, index) => {
                ...
                // Remove a parte entre parênteses para não quebrar a URL (ex: "(França)")
                const nomeSemPais = opcao.replace(/\s*\(.*?\)\s*/g, '').trim();
                const playerUrlName = nomeSemPais.toLowerCase().replace(/\s+/g, '.');
                  ...
                  >
                    ...
                    <div className={`w-20 h-20 md:w-24 md:h-24 mb-4 rounded-full overflow-hidden shrink-0 ${ringColor} bg-[#051020] relative flex items-center justify-center`}>
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(nomeSemPais)}&background=10B981&color=fff&bold=true`;
                        }}
                      />
                    </div>
